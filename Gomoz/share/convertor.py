...
def main(argv=None):
    ...
    png_files=glob(os.path.join("D:\marnic\lib\cc\img",'*.png'))
	...
	#v = re.sub('_{2,}', '_', v)
        ...
        r=split_string(convert_base64(s,v))
        s_vars+=r+"\n\n"

    s_names=split_list("_names="+str(v_list))+"\n\n"

    if os.path.exists("D:\marnic\lib\cc\img\images.py"):
        try:
            os.rename("D:\marnic\lib\cc\img\images.py","D:\marnic\lib\cc\img\images.bak")
            print "Your old images.py was renamed images.bak"
        except:
            print "Sorry, I don\'t do deletes..."
            print "Please delete images.bak yourself"

    f=open("D:\marnic\lib\cc\img\images.py","w")
    f.write(s_header)
    f.write(s_names)
    f.write(s_vars)
    f.close()
